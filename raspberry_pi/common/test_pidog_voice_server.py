...
REPOSITORY_PI_DIR = Path(__file__).resolve().parent.parent
...
class ServerTest(unittest.TestCase):
    ...
    def test_server_accepts_every_android_command(self):
        source_path = Path(os.environ.get("PIDOG_ANDROID_COMMAND_SOURCE", ""))
        ...
        robot_command_source = source_path.read_text(encoding="utf-8")
...
class AssistantBackendTest(unittest.TestCase):
    def test_uses_backend_api_for_variant(self):
        response = Mock()
        response.__enter__ = Mock(return_value=response)
        response.__exit__ = Mock(return_value=False)
        if TEST_VARIANT == "ai_hat_2":
            response.read.return_value = json.dumps({
                "message": {"content": "Привет от Hailo"},
            }).encode("utf-8")
            expected_url = "http://127.0.0.1:8000/api/chat"
            expected_model = "qwen2:1.5b"
        else:
            response.read.return_value = json.dumps({
                "choices": [{"message": {"content": "Привет от CPU"}}],
            }).encode("utf-8")
            expected_url = "http://127.0.0.1:8081/v1/chat/completions"
            expected_model = "Qwen3.5-2B Q4_K_M"

        with patch("pidog_voice.assistant.urlopen", return_value=response) as open_url:
            answer = AssistantManager()._complete("Привет", [])

        self.assertIn("Привет", answer)
        request = open_url.call_args.args[0]
        self.assertEqual(expected_url, request.full_url)
        self.assertEqual(expected_model, json.loads(request.data)["model"])
