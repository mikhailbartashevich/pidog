...
class AssistantManager:
    """Talks to the loopback-only Hailo Ollama server and fixed search backends."""

    def __init__(self, dry_run: bool = False) -> None:
        self._dry_run = dry_run
        self._base_url = os.environ.get("PIDOG_LLM_URL", "http://127.0.0.1:8000").rstrip("/")
        self._unit = os.environ.get("PIDOG_LLM_UNIT", "pidog-hailo-ollama.service")
        self._model_name = os.environ.get("PIDOG_LLM_NAME", "qwen2:1.5b")
        self._ready_path = os.environ.get("PIDOG_LLM_READY_PATH", "/hailo/v1/list")
        self._systemd_scope = os.environ.get("PIDOG_SYSTEMD_SCOPE", "system")
        self._server_name = os.environ.get("PIDOG_LLM_SERVER", "hailo-ollama")
        ...
        # Keep just the two most recent exchanges. Short history keeps latency and
        # Hailo-10H memory use predictable while preserving conversational context.
    ...
    def status(self) -> dict[str, Any]:
        ...
        with self._status_lock:
            ...
            installed = shutil.which(self._server_name) is not None
    ...
    def control(self, action: str) -> dict[str, Any]:
        ...
        if shutil.which(self._server_name) is None:
            raise AssistantUnavailableError("hailo-ollama ещё не установлен")
        try:
            systemctl = ["systemctl"]
            if self._systemd_scope == "user":
                systemctl.append("--user")
            completed = subprocess.run(
                [*systemctl, action, self._unit],
    ...
    def _complete(self, question: str, sources: list[dict[str, str]]) -> str:
        ...
        request = Request(
            f"{self._base_url}/api/chat", data=payload, method="POST",
    ...
    def _llm_ready(self, timeout: float) -> bool:
        try:
            with urlopen(f"{self._base_url}{self._ready_path}", timeout=timeout) as response:
    ...
    def _systemd_state(self) -> str:
        try:
            completed = subprocess.run(
                [
                    "systemctl",
                    *( ["--user"] if self._systemd_scope == "user" else [] ),
                    "is-active", self._unit,
                ],
